...
const TEMPLATES = {
  ...
  socialTerryRun: {
    ...
    edgeColor: '#1A2332',
    layers: [
      { id:'city',      type:'text',   x:0.5000,y:0.0422, align:'center', font:'700 44px sans-serif', color:'ink', maxW:0.9 },
      ...
      { id:'footer',    type:'social_footer', x:0.2537,y:0.9820, align:'left', font:'400 20px sans-serif', boldFont:'700 20px sans-serif', color:'ink', maxW:0.4685, lineHeight:18 },
  ...
  if (currentUser) {
    container.innerHTML = `
      <div style="font-size: 13px; font-weight: 500; display:flex; align-items:center; gap:8px;">
        <img src="${currentUser.photoURL}" style="width:24px; height:24px; border-radius:50%;" onerror="this.style.display='none'">
        ${currentUser.displayName ? currentUser.displayName.split(' ')[0] : 'Admin'}
      </div>
      <button class="btn btn-secondary" onclick="showDraftsModal()" style="padding: 6px 12px; font-size:12px;">My Drafts</button>
      <button class="btn btn-secondary" onclick="window.firebaseAuth.signOut()" style="padding: 6px 12px; font-size:12px;">Log Out</button>
    `;
    if (saveBtn) saveBtn.style.display = 'block';
  } else {
    container.innerHTML = `
      <button class="btn btn-primary" onclick="loginWithGoogle()" style="padding: 6px 12px; font-size:12px;">Admin Login</button>
    `;
...
function loginWithGoogle() {
  window.firebaseAuth.signInWithPopup(window.firebaseProvider).catch(error => {
...
async function saveDraftToCloud() {
  ...
  const eventName = await showPromptModal('Save Draft', 'Enter a name for this event:', 'e.g. NYC Terry Fox Run 2026');
  if (!eventName) return;
  ...
  try {
    const draftRef = window.firebaseDb.collection('drafts').doc();
...
async function showDraftsModal() {
  ...
  modal.innerHTML = `
    <h3 style="margin: 0 0 16px 0; font-size: 18px; font-weight: 600; font-family: var(--font-ui);">My Saved Drafts</h3>
    <div id="draftsListContainer" style="max-height: 400px; overflow-y: auto; margin-bottom: 20px;">
      <div style="text-align:center; padding: 20px; font-family: var(--font-ui); color: #6B7280;">Loading...</div>
    </div>
    <div style="display: flex; justify-content: flex-end;">
      <button class="custom-modal-btn" id="closeDraftsBtn">Close</button>
    </div>
  `;
  overlay.appendChild(modal);
  document.body.appendChild(overlay);
  
  overlay.querySelector('#closeDraftsBtn').addEventListener('click', () => {
    document.body.removeChild(overlay);
  });

  try {
    const draftsRef = window.firebaseDb.collection('drafts');
    const q = draftsRef.where('userId', '==', currentUser.uid);
    const snapshot = await q.get();
    const container = overlay.querySelector('#draftsListContainer');
    
    if (snapshot.empty) {
      container.innerHTML = '<div style="text-align:center; padding: 20px; font-family: var(--font-ui); color: #6B7280;">No saved drafts found.</div>';
      return;
    }
    ...
    drafts.forEach(draft => {
      ...
      item.style.cssText = 'padding: 12px; border: 1px solid var(--border); border-radius: 8px; margin-bottom: 8px; cursor: pointer; display: flex; justify-content: space-between; align-items: center; font-family: var(--font-ui);';
      item.innerHTML = `
        <div>
          <div style="font-weight: 600; color: #111827;">${draft.eventName || 'Unnamed Draft'}</div>
          <div style="font-size: 12px; color: #6B7280;">Template: ${draft.templateId}</div>
        </div>
        <button class="btn btn-primary" style="padding: 6px 12px; font-size: 12px;">Load</button>
      `;
